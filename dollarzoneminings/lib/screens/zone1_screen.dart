...
class Zone1Screen extends StatefulWidget {
  const Zone1Screen({super.key});

  @override
  State<Zone1Screen> createState() => _Zone1ScreenState();
}

class _Zone1ScreenState extends State<Zone1Screen> {
  ...
  final List<String> fakeAdUnitIds = [
    'ca-app-pub-fake-0001/rewarded_ad_unit_01',
    'ca-app-pub-fake-0002/rewarded_ad_unit_02',
    'ca-app-pub-fake-0003/rewarded_ad_unit_03',
    'ca-app-pub-fake-0004/rewarded_ad_unit_04',
    'ca-app-pub-fake-0005/rewarded_ad_unit_05',
    'ca-app-pub-fake-0006/rewarded_ad_unit_06',
    'ca-app-pub-fake-0007/rewarded_ad_unit_07',
    'ca-app-pub-fake-0008/rewarded_ad_unit_08',
    'ca-app-pub-fake-0009/rewarded_ad_unit_09',
    'ca-app-pub-fake-0010/rewarded_ad_unit_10',
  ];
  ...
  Future<void> _loadZoneTotals() async {
    ...
    final z = (snap.data() ?? {})['zone1'] as Map<String, dynamic>?;
    if (z != null) {
      setState(() {
  ...
  Future<void> _moveCoinsToWalletAndResetZone() async {
    ...
    final zoneData = (doc.data() ?? {})['zone1'] as Map<String, dynamic>? ?? {};
    ...
    await _fire.runTransaction((tx) async {
      ...
      if (!walletSnap.exists) {
        tx.set(walletRef, {
          'balance': zoneCoins / 100, // conversion: 100 coins = $1 (change if you want)
          'earnings': zoneCoins,
      ...
      tx.update(docRef, {
        'zone1.coinsEarned': 0,
        'zone1.buttonsCompleted': 0,
        'zone1.cyclesDoneToday': 0,
        'zone1.buttons': {},
      });
    });
  ...
  Future<void> _onEarned(double coins) async {
    // coins is in coin units (0.5 etc)
    // update local display + Firestore aggregate
    ...
    // increment zone1.coinsEarned in Firestore
    await docRef.set({
      'zone1': {
    ...
    // also update wallet minimal immediate tracking (earnings and balance)
    final walletRef = _fire.collection('wallets').doc(uid);
    await _fire.runTransaction((tx) async {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Zone 1 Mining"),
      ),
      body: Padding(
        ...
        child: Column(
          children: [
            Text(
              "Zone 1 - Total Coins: ${totalCoins.toStringAsFixed(2)}",
